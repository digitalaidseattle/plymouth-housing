...
export default function TicketsGrid() {
  ...
  useEffect(() => {
    if (paginationModel && sortModel) {
      const queryModel = {
        ...
        sortDirection:
          sortModel.length === 0 ? 'created_at' : sortModel[0].sort,
      } as QueryModel;
      ticketService.query(queryModel).then((pi) => setPageInfo(pi));
    }
  ...
  const applyAction = () => {
    alert(
      `Apply some action to ${rowSelectionModel ? rowSelectionModel.length : 0} items.`,
    );
  };
  const newTicket = () => {
    alert(`New Clicked`);
