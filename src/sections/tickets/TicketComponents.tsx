...
const TicketLink: React.FC<TicketProps> = ({ ticket }) => {
  return (
    <Link color="secondary" component={RouterLink} to={`/ticket/${ticket.id}`}>
      {ticket.id}
    </Link>
  );
};
...
const TicketLongForm: React.FC<TicketFormProps> = ({
  ticket,
  staff,
  messages,
  onChanged,
    ...
    <MainCard title="History">
      <Stack spacing={'1rem'}>
        {ticket.ticket_history
          .sort((h1: TicketHistory, h2: TicketHistory) =>
            moment(h2.created_at).diff(moment(h1.created_at)),
          )
          .map((hist: TicketHistory, idx: number) => {
            ...
            return (
              <MainCard key={idx}>
                ...
                <Typography>
                  Date: {date.format('MM-DD-YYYY')} {date.format('hh:mm')}
                </Typography>
