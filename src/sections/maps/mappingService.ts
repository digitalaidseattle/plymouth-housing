...
const LOCATIONS: Location[] = [
  ...
  {
    name: 'Mission Viejo, California, USA',
    latitude: 33.5969,
    longitude: -117.6582,
  },
  ...
  { name: 'Redmond, WA', latitude: 47.674, longitude: -122.1215 },
  { name: 'Redmond, WA, USA', latitude: 47.674, longitude: -122.1215 },
  { name: 'Redmond, Washington', latitude: 47.674, longitude: -122.1215 },
...
class MappingService {
  ...
  async getPeople(): Promise<TeamMember[]> {
    ...
    return airtableService
      .getTableRecords(TABLE, MAX_RECORDS, FILTER)
      .then((records) =>
        records.map((r) => {
  ...
  unique(locations: Location[]): Location[] {
    let unique = locations.reduce((arr: Location[], b: Location) => {
      if (
        !arr.find(
          (test) =>
            test.longitude === b.longitude && test.latitude === b.latitude,
        )
      ) {
        arr.push(b);
      }
