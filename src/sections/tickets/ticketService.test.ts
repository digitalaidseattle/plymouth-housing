...
describe('ticketService tests', () => {
  it('query', async () => {
    ...
    const selectSpy = vi
      .spyOn(mockQueryBuilder, 'select')
      .mockReturnValue(mockFilterBuilder as any);
    const rangeSpy = vi
      .spyOn(mockFilterBuilder, 'range')
      .mockReturnValue(mockFilterBuilder as any);
    const orderSpy = vi
      .spyOn(mockFilterBuilder, 'order')
  ...
  it('createTicket', async () => {
    ...
    const fromSpy = vi
      ...
      .mockReturnValue(mockQueryBuilder as any);
    const insertSpy = vi
      .spyOn(mockQueryBuilder, 'insert')
      .mockReturnValue(mockQueryBuilder as any);
    const selectSpy = vi
      .spyOn(mockQueryBuilder, 'select')
      .mockReturnValue(Promise.resolve(response));
    const createTicketHistorySpy = vi
      .spyOn(ticketService, 'createTicketHistory')
  ...
  it('updateTicket', async () => {
    ...
    const fromSpy = vi
      ...
      .mockReturnValue(mockQueryBuilder as any);
    const updateSpy = vi
      .spyOn(mockQueryBuilder, 'update')
      .mockReturnValue(mockQueryBuilder as any);
    const eqSpy = vi
      .spyOn(mockQueryBuilder, 'eq')
      .mockReturnValue(mockQueryBuilder as any);
    const selectSpy = vi
      .spyOn(mockQueryBuilder, 'select')
      .mockReturnValue(Promise.resolve(response));
    ...
    expect(createTicketHistorySpy).toHaveBeenCalledWith({
      ...
      description:
        'Changed "clientName" to "clientName"\nChanged "description" to "desc"',
      change_by: 'fff',
    });
