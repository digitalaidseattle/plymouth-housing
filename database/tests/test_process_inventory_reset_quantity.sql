...
-- TEST 4: Invalid item ID (should fail)
PRINT '--- TEST 4: Invalid Item ID (Should Return Error) ---';
...
IF EXISTS (SELECT 1 FROM #InvalidItemResult WHERE Status = 'Error')
    PRINT 'PASS: Invalid item properly detected and rejected';
ELSE
...
PRINT '--- TEST 5: Negative Quantity (Should Return Error) ---';
...
-- Check if we got an error status
IF EXISTS (SELECT 1 FROM #NegativeQtyResult WHERE Status = 'Error')
    PRINT 'PASS: Negative quantity properly detected and rejected';
ELSE
    PRINT 'FAIL: Expected Error status not returned';
...
-- TEST 6: Reset to zero (should succeed)
PRINT '--- TEST 6: Reset to Zero (Valid Case) ---';
...
-- TEST 7: Verify transaction type is CORRECTION (type 3)
PRINT '--- TEST 7: Verify Transaction Type ---';
SELECT 'All test transactions (should be type 3 = CORRECTION):' AS Description,
    ...
    END AS TypeCheck
FROM Transactions
WHERE id IN (@txn_id_1, @txn_id_2, @txn_id_5)
ORDER BY transaction_date;
PRINT '';
